...
bot.on('guildMemberAdd', member =>{
    ...
    .setDescription(`Hey ! Salut à toi, ${member.user} (${member.user.name})! Tu es ici sur le serveur discord de la communauté ${member.guild.name}, tu es invité à lire le ${reglement} . Tu as pour le moment un rôle restreint afin d'avoir accès à tout le discord nous t'invitons à faire une petite ${presentation} puis à contacter un membre du Staff pour l'entretien !`);
...
bot.on('guildMemberRemove', member =>{
    ...
    .setDescription(`${member.user} ( ${member.user.name} ) s'est barré !!`)
